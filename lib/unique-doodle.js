/**
 * Sample use:
 * <svg fill="none" width=40 height=40 viewbox="0 0 40 40" stroke="black"><path :d="getUniqueDoodlePath(uniqueString)"/></svg>
 */
function getUniqueDoodlePath(baseStr, width, height) {
    ...
    const length = minLength + generator.getInt(maxLength - minLength);
...
/**
 * Sample use:
 * document.body.appendChild(getUniqueDoodleSvg());
 */
...
export {getUniqueDoodlePath, getUniqueDoodleSvg, DoodleGenerator, RandomGenerator};
...
class DoodleGenerator {
    ...
    #getInt(max) {
        if (this.#randomGenerator) {
            return this.#randomGenerator.getInt(max);
        }
    ...
    #failCount = 0;
    #getPoint() {
        let x = this.#getInt(this.#width * 0.6) + this.#width * 0.2;
        let y = this.#getInt(this.#height * 0.6) + this.#height * 0.2;
        ...
        if (this.#lastTwoPoints.length < 2) {
            ...
            let newPointIsValid = DoodleGenerator.getAngle(...this.#lastTwoPoints, point) >= this.#minAngle
            ...
            if (this.#failCount > 100) { newPointIsValid = true; } // We are stuck in a corner, giving up on keeping things smooth

export default class MidiInput extends EventTarget {
    ...
    /**
     * useful in particular for the calling code to know if midi input is available / expected
     */
    hasActiveDevices() {
        for (const deviceId in this.#midiDevices) {
            if (this.#midiDevices[deviceId].useIfAvailable && this.#midiDevices[deviceId].status == 'online') {
                return true;
            }
        }
        return false;
    ...
    async startListening(midiDevice) {
        // Not sure if that's the same for everone but my midi keyboard does not send events until it has been used as an output once.
        // Doing so now, in case it helps.
        // Unfortunately, there is no way to know what output matches an input...
        // Sending to every output, for now.
        if (!midiDevice) { console.error('Error connecting midi device: param is null'); return; }
        midiDevice.onmidimessage = this.midiEventsHandler.bind(this);

        await new Promise(r => setTimeout(r, 200));

        const midiAccess = await this.getMidiAccess();
        midiAccess.outputs.forEach((o) => { o.send([0x90, 60, 0x03]) });
    }
    ...
    midiEventsHandler(event) {
        ...
        */
        ...
        const pianoKey = key - 8;
        // should be key - 20 according to this??? https://studiocode.dev/resources/midi-middle-c
        // Might it be different for various midi devices??

        if (command === this.MIDI_KEYDOWN) {
            // TODO: handle chords: add to local list of "currently down" keys, remove on keyup - or handle in music-input?
            this.dispatchEvent(new CustomEvent('press', { detail: {key: key - 8} }));
            // this.dispatchEvent(new CustomEvent('chord', { detail: {key: key} }));
        } else if (command === this.MIDI_KEYUP) {

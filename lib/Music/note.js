...
export default class Note {
    ...
    */
    ...
    get spn() {
        if (this.#spn == undefined && this.#keyNum != undefined) {
            this.#spn = Note.numToSpn(this.#keyNum);
        }
        return this.#spn;
    }
    ...
    static numToSpn(noteNum) {
        const noteInOctave = this.#positiveModulo(noteNum, 12); // noteNum % 12 but with real modulo instead of JS's negative-friendly oddity
        const octave = Math.floor(noteNum / 12) + 4; // since middle C a.k.a. note 0 should be C4
        const sharp = [1, 3, 6, 8, 10].indexOf(noteInOctave);
        ...
        const aIndexed = this.#positiveModulo(adjustedNote + 2, 7); // +2 to go from C=0 to A=1
        let spnNote = String.fromCharCode(97 + aIndexed);
        ...
        console.log({noteNum, noteInOctave, octave, sharp, adjustedNote, spnNote});
        return spnNote;
    }
